...
void setup() {
  ...
  modbus.begin(2, 115200);  // slave id, baud rate, config (optional)
  dmd.begin();
}
...
void drawText(uint16_t* values){
    ...
    for(int i=0;i<width;i++){
      ...
      int padLeft = 0; 
      if (data < 100){
        padLeft = CalPixelXYFloat2_Droid(data);
        dmd.selectFont(Droid_Sans_16);
        dmd.drawString((32*i) + padLeft,1,String(data * 0.1,1),GRAPHICS_ON);
      }else if (data > 99 && data < 1000){
        padLeft = CalPixelXYFloat3_Droid(data);
        dmd.selectFont(Droid_Sans_16);
        dmd.drawString((32*i) + padLeft,1,String(data * 0.1,1),GRAPHICS_ON);
...
int CalPixelXYFloat2_Droid(int n)
{
  int m = n;
  int r = 100;
  int len = 2;
  ...
  while(len>0){
    int x = (m % r) / (r/10);
    r /= 10;
    sum += width_num_droid[x];
    len--;
  }
  return 32 - (sum + 4);
}
int CalPixelXYFloat3_Droid(int n)
{
  int m = n;
  int r = 1000;
  int len = 3;
